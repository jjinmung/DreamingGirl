using System;
using System.Threading.Tasks;
using Cysharp.Threading.Tasks;
using DG.Tweening;
using Unity.Cinemachine;
using UnityEngine;

public class Store : MonoBehaviour,IInteractable
{
    public Transform NPC;
    bool IsInteracting = true;
    Vector3 defalutRoation = new Vector3(0,90,0);
    Vector3 InteractionRotation = new Vector3(0,60,0);
    public bool IsInteractable => IsInteracting;

    private void Start()
    {
        IsInteracting = Managers.Data.IsNew;
    }

    public void OnInteract()
    {
        Managers.Camera.SetStoreCam(IsInteracting);
        Managers.Player.Control.InputActive(IsInteracting);
        NPC.GetComponent<Animator>().SetTrigger("Interacting");
        NPC.DORotate(InteractionRotation, 1f);
        IsInteracting = !IsInteracting;
        Invoke(nameof(DelayDialoge),2f);
    }

    void DelayDialoge()
    {
        Managers.Dialogue.StartDialogue("3");
    }

    public void EndDialogue()
    {
        Managers.Camera.SetStoreCam(IsInteracting);
        Managers.Sound.PlayBgm(Managers.Resource.Data.StoreMapBGM).Forget();
        Invoke(nameof(DelayShowUI),1f);
    }

    void DelayShowUI()
    {
        var battleUI = Managers.UI.SceneUI as UI_BattleScene;
        if (battleUI != null)
            battleUI.LobyUIActive();
        
    }
}   
